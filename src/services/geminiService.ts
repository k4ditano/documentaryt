...
import { Page, Folder } from '../types/index';
import { Task, TaskStatus, TaskPriority, taskService } from './taskService';
      ...
      try {
        if (typeof page.content === 'string') {
          ...
          if (Array.isArray(parsedContent)) {
              textContent = parsedContent.map((block: TextBlock) => {
          ...
          textContent = page.content.map((block: TextBlock) => {
            if (block.content && Array.isArray(block.content)) {
              return block.content.map((c: { text?: string }) => c.text || '').join(' ');
            } else if (block.text) {
              return block.text;
      ...
      return {
        results: Array.isArray(parsedResponse.results) ? 
          parsedResponse.results
            .filter((result: SearchResult) => result && result.relevance && result.relevance > 0.3)
...
export const analyzePDF = async (pdfText: string): Promise<PDFAnalysis> => {
  try {
...
const processTextStyles = (text: string) => {
  let styles: any = {};
...
export const generateContent = async (title: string): Promise<string> => {
  try {
    const prompt = `
...
const formatTaskList = async (tasks: Task[]): Promise<string> => {
  ...
  const tasksByPriority: { [key: string]: Task[] } = {
    high: [],
    medium: [],
    low: []
  };
  ...
  try {
    ...
    switch (actionResponse.action) {
      case 'create_page':
        ...
        const cleanContent = actionResponse.details.content.map((block: any) => ({
      ...
      case 'create_task':
        ...
        const taskDetails: Omit<Task, "id" | "created_at" | "updated_at" | "user_id"> = {
...
const formatPageContent = (content: any): string => {
...
export const createPage = async (title: string, content: string): Promise<void> => {
  try {
    ...
    contentArray = contentArray.map((block: any) => ({
      type: block.type || 'paragraph',
      props: {
        textAlignment: block.props?.textAlignment || 'left',
        backgroundColor: 'default',
...
export const createTask = async (taskDetails: Omit<Task, "id" | "created_at" | "updated_at" | "user_id">): Promise<void> => {
  try {
...
export const suggestRelatedTags = async (existingTags: string[]): Promise<string[]> => {
  try {
    const prompt = `
