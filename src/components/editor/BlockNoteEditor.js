...
const BlockNoteEditor = ({ content, onChange }) => {
    const [initialContent, setInitialContent] = useState([createDefaultBlock()]);
            ...
            if (Array.isArray(parsedContent) && parsedContent.length > 0) {
                ...
                const validBlocks = parsedContent.map(block => {
                            ...
                            if (Array.isArray(block.content)) {
                                const tableContent = {
                                    ...
                                    rows: block.content.map((row) => ({
                                        cells: row.content.map((cell) => Array.isArray(cell.content) ? cell.content : [{
                                                type: "text",
                                                text: cell.content || "",
                                                styles: cell.styles || {}
                                            }])
                    ...
                    return {
                        ...
                        content: blockContent.map((content) => ({
    ...
    useEffect(() => {
        if (!editor || !initialContent.length || isUpdatingRef.current)
            return;
        try {
            const savedPosition = cursorPositionRef.current;
            isUpdatingRef.current = true;
    ...
    return (_jsx("div", { style: { flex: 1, position: 'relative', height: '100%' }, className: "bn-container", children: _jsx(BlockNoteView, { editor: editor, theme: notionLightTheme }) }));
};
